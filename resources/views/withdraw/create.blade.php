...
<body>
            ...
            <form action="{{ route('withdraw.store') }}" method="POST">
                ...
                <div class="form-check" style="margin-bottom: 10px">
                    @foreach ($exchanges as $ex)
                        <input class="form-check-input" type="radio" required value="{{ $ex->name }}"
                            name="type">
    ...
    <script src="{{ asset('public/js/select2cdn.js') }}"></script>
    <script>
        ...
        $('#details_id').change(function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#hamada').html("اخر تاريخ صرف بتاريخ : " + response['date'] + "<br>" +
                        " العدد المصروف :  " + (response['value']) + "<br>" + " نوع الصرف : " +
                        response['type']);
